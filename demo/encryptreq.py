...
def http_submit():
    ...
    try:
        ...
        Parameter.insert(END, dumps(jsons, indent = 4)+"\n")
    except:
...
Label(master, text="Password：", font="bold").grid(row=1)
Label(master, text="Parameter：", font="bold").grid(row=2)
Label(master, text="Responses：", font="bold").grid(row=4)

ServerURL = Entry(master, width = 95, bg = "#cce8cf")
Password  = Entry(master, width = 95, bg = "#cce8cf", show = "*")
...
ResponsesText     = Text(
    ResponsesFrame, bg = "#cce8cf", fg="#5200f7", height=25, width=92, 
...
Parameter.grid(row=2, column=1, padx=10, pady=10)
ResponsesFrame.grid(row=4, column=1, pady=10)

#submit button : row=3
Button(
    master, text='Submit', fg="#ffffff", bg="#21aeba", command=http_submit
).grid(
    row=3, columnspan=2, ipadx=20, pady=10
)
